...
const attachBackgroundAnimation = ( container, index ) => {
	...
	const ambientClass = createAnimationStyle(
		'smoothmg-bg--ambient',
		( c ) => `
    .${ c } {
  ...
  `
	...
	const renderFrame = () => {
		...
		ambient.querySelector(
			'.smoothmg-bg--ambient-light__tl'
		).style.opacity = 0.2 + smoothMid * intensity;
		ambient.querySelector(
			'.smoothmg-bg--ambient-light__tr'
		).style.opacity = 0.2 + smoothHigh * intensity;
		ambient.querySelector(
			'.smoothmg-bg--ambient-light__bl'
		).style.opacity = 0.2 + smoothBass * intensity;
		ambient.querySelector(
